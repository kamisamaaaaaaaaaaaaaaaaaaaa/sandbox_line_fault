...
| `inject.fault.times` | 否 | **`1`** | **每行每线程每调用栈故障次数**（≥1）。同一行在同一线程、经同一调用栈执行时本轮最多发生多少次故障；该行被 T 个线程、S 种调用栈执行时本轮最多 T × S × N 次。同一行号对应多段代码时，调大 N 可让各段代码位置逐一触发（见 [9.2](#92-同一行号对应多段代码与-injectfaulttimes)）。填入 <1 的值即硬保护 |
...
| `thread.include` | 否 | **空**（全部线程参与） | 线程名选入正则（对线程名全串匹配，一行一个）。空 = 全部线程参与故障 |
| `thread.exclude` | 否 | **空**（不排除） | 线程名排除正则。用于排除名字不稳定的噪音线程（监控/心跳/定时任务等）；被过滤的线程执行到注入行时静默放行（不写表3、不 kill），过滤是运行期行为，不影响注册数 |
...
### 9.1 表2 `code_lines` 的行号归属规则

`LineNumberTable` 是 javac 生成的「字节码指令 → 源码行号」映射：**行号属于指令，不属于语法关键字**。
表2 的 `code_lines` 统计的是**出现过的去重行号数**，一行是否计入取决于该行是否承载了指令：

| 源码构造 | 是否计入 | 说明 |
|---|---|---|
| 注释、空行、单独 `{` | 否 | 不产生指令 |
| `if (cond)` / `else if (...)` 行 | 是 | 条件跳转指令（`ifeq` 等）落在该行 |
| 单独 `else` / `} else {` 行 | 否 | else 分支的首条指令落在块内语句行 |
| `try {` 行 | 否 | try 块内首条指令落在块内语句行 |
| `} catch (X e) {` 行 | 是 | 异常处理器入口的 `astore` 落在该行 |
| `finally` 块内语句 | 是 | 编译器把 finally 复制到各出口路径，行号仍指向 finally 内源码行（同一行号出现多次，去重算 1） |
| try/finally 结构结尾 `}` | 是 | 编译器合成的 `goto`（跳过异常处理器）与异常路径的 `athrow` 落在该行 |
| 方法结尾 `}`（无显式 return） | 是 | 编译器补的隐式 `return` 落在该行 |
| 方法结尾 `}`（有显式 `return xxx;`） | 否 | 返回指令标注在 return 语句行 |
| 显式 `return xxx;` / 裸 `return;` 行 | 是 | 返回指令在该行 |
| 一条语句跨多行 | 只计起始行 | javac 把整条表达式归到起始行，续行不产生条目 |
| `switch` 的 `case N:` / `default:` 标签行 | 否 | 标签行本身无指令，计入的是 case 内的语句行 |
| `for` / `while` 行 | 是 | 条件判断与自增指令落在该行 |
| 三元运算符、一行多条语句 | 是（该行计 1） | 同一行多个指令段去重算 1 行 |

同一行号可能在 `LineNumberTable` 中出现多次（一条语句被拆成多段指令、中间插入其他行的指令），
`code_lines` 取**去重**行号数——它既不等于指令段数，也不等于源码物理行数。

### 9.2 同一行号对应多段代码与 `inject.fault.times`

织入器在 `LineNumberTable` 的**每个条目**处都插入行级探针。同一行号出现多个条目时
（典型如一条链式调用折行书写，中间穿插了其他行的指令），该行的**多个代码位置都会触发 `beforeLine`，
且上报的行号相同**。此时：

- `inject.fault.times = 1`（默认）：该行第一次触发即抢占故障（`fault_seq=1`）并 kill 当前进程，
  该行的其余代码位置不会再执行到——同一行本轮只留 1 条故障记录；
- 要让**同一行号的不同代码位置**各自触发一次故障，把 `inject.fault.times` 调大为 N：
  第 1 次触发抢占 `fault_seq=1` 并 kill；下一轮该行放行后执行到同一行的下一个代码位置，
  继续抢占 `fault_seq=2`……直到 N 次用尽。于是同一行在表3 中会出现 `line_no` 相同、`fault_seq` 递增的多条记录。

> 注意：`fault.times` 的计数粒度是「行 + 线程 + 调用栈」，N 次机会按触发顺序消费，不区分该行内的具体代码位置。
