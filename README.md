...
| A5 | 表1 登记 | 裸 INSERT 唯一键冲突（sha256 已存在）但紧接着 `findBySha256` 查不到行 = 数据不一致 | 硬保护 `DB` | 无 | kill |
| A5 | 表1 登记 | **非冲突 SQL 错误**（截断/非法值等，SQLState 非 23 类） | 硬保护 `DB`（冲突判定基于 JDBC 标准 SQLState 23 类，不再用 INSERT IGNORE 静默吞错） | 无 | kill |
...
| A6 | 表2 落库 + 置 completed | 批量插入遇唯一键冲突（多节点并发解析同单元） | 该批回滚**降级逐行**：冲突行跳过、其他错误硬保护（携带该单元 id） | 收敛为并集 | 继续 |
| A6 | 表2 落库 + 置 completed | 批量插入遇**非冲突 SQL 错误**（截断/非法值等） | 硬保护 `DB`（携带该单元 id） | 该单元保持未完成 | kill |
...
| C3 | 裸 INSERT 成功 = 赢得本行本轮执行权 | 日志 → `kill -9` 自己 | **进程终止** |
| C4 | `kill` 命令未生效 | 表4 留痕 + **回滚删除表3 记录**（防脏数据永久阻塞该行） + 移出缓存 | 放行继续 |
| C5 | 唯一键冲突（SQLState 23 类）= 集群内他节点已触发该行；**其他 SQL 错误 → 硬保护** | 加入本地缓存 | 放行继续（仅冲突） |
