...
批量写库遇冲突批（并发解析同单元）自动降级逐行：冲突行跳过、非 23 类 SQL 错误立即上抛硬保护；**降级重放会计数并在 unit stored 日志中报告冲突跳过行数**（`conflict-skipped=N rows already existed`，用于观测并发/重解析时的重复写入量）。

**SQLState 23 类判定的适用性边界**：SQLState 分类码由 ANSI SQL/JDBC 标准定义，`23` 类 = 约束违反，**MySQL（23000）/ PostgreSQL（23505）/ SQL Server / DB2 / H2 均适用**；**Oracle 不守规范**（ORA-00001 常映射为 99999）、SQLite 老驱动 SQLState 为 null——这些库上唯一键冲突会被误判为非冲突走硬保护（失败方向安全，但功能不可用），当前部署目标为 MySQL 不受影响。另：23 类含外键/NOT NULL 违反，但本工具表无外键、插入参数由代码保证非 null，23 类实际只可能来自唯一键冲突，语义精确。
