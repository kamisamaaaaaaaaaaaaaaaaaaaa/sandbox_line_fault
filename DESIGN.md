...
- **判重键 = 轮次 + 应用（bootJar 部署路径）+ 行 + 线程 + 第几次故障**：表3 唯一索引
  `uk(tag, boot_jar_hash, class_name, method_name, line_no, thread_name, fault_seq)`。
  同一行在同一线程下本轮最多发生 `inject.fault.times` 次故障（未配置时默认 1）；该行被 T 个线程执行时本轮最多 T × N 次。
  ...
  - **索引键用 16 位 hash 而非完整路径**：① 索引长度上限仅 274 字符，超长路径会导致插入即硬保护；② 默认 collation 不区分大小写，会误判 Linux 上大小写敏感的路径。16 位 hash（64bit）碰撞概率可忽略。
  - **判重键必须包含应用标识**：表1 以 **sha256（内容）** 为单元键，同一 jar 部署在两个不同路径时共用同一 `unit_id`；判重键加入 `boot_jar_hash` 后，两个部署副本各自独立抢占、独立 kill，互不干扰。
- **故障次数计数（模块侧）**：按「行+线程」维护已确认的故障次数。
  ...
  - 进程被 kill 后计数随之消失，新进程从 0 开始，靠插入冲突逐次推进——因此线程跑满 N 次需跨多个进程生命周期累计执行该行约 `1+2+…+N` 次。
...
- **原子性**：判重完全依赖数据库唯一索引（裸 INSERT 的冲突判定为单语句原子操作），并发同抢同一「行+线程+第几次」仅 1 条成功——不依赖应用内锁，多机多实例天然安全。
...
| `t_fault_record` | 故障命中（含轮次 tag、第几次故障） | **`uk(tag, boot_jar_hash, class, method, line, thread_name, fault_seq)`** 轮内抢占，粒度为「行+线程+第几次」；`ip` 为普通列仅记录死亡节点，不参与判重 |
| `t_error_record` | 工具自身错误 | — |

> 索引长度核算（表3 唯一键，utf8mb4）：tag 64×4 + boot_jar_hash 16×4 + class 256×4 + method 128×4 + line 4 + thread_name 128×4 + fault_seq 4 = **2376B < 3072B**，安全。

---

## 5. 覆盖率统计口径

覆盖率按**可达行**统计：javap 基线行号全集大于运行时可达行，差集归因于三类——未实例化类的构造器行、运行时不可达分支（基线噪声）、sandbox 织入盲区（`main` 方法与构造器 `super()` 前的行，见 [3.3.3](#333-织入边界影响覆盖率口径)）。

表3 的记录数不等于覆盖行数（同一行每线程最多 N 条），覆盖率按 `COUNT(DISTINCT class_name, method_name, line_no)` 统计；按「行+线程+第几次」核对时，每个组合应为 N 条且 `fault_seq` 从 1 连续无缺号。
