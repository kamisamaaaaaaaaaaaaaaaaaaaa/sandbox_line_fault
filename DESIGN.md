...
- **运行期的线程名过滤**（`thread.include` / `thread.exclude`）：与注入过滤块正交的第二个维度——注入决定"哪些行会触发"，线程过滤决定"哪些线程执行到该行时触发"。`beforeLine` 中在取栈**之前**判定当前线程名（先 include 选入，空 = 全选，再 exclude 排除，正则对线程名全串匹配），不通过的线程**静默放行**——不写表3、不 kill、不打日志。定位是排除名字不稳定的噪音线程（监控/心跳/定时任务等）；过滤是运行期行为，不影响解析收录与注册数，因此也不参与零覆盖判定。每次回调直接对预编译 Pattern 匹配、不做结果缓存——线程名可被运行时修改（`Thread.setName`），缓存会让判定陈旧失真；不通过的线程连取栈开销都省掉，匹配本身远小于取栈。
